...
#other expired
sqlite3 -separator ' ' $database "select distinct infos.id, category.name from infos, category where category.id=infos.exp_action and DateTime()>DateTime(expiration);"	> $ramdisk/expired
