...
( 
  ...
  id in (select category_id from category_file where item_id=$id)
  OR
  id in (select distinct parent from category where id in (select category_id from category_file where item_id=$id) )
  OR
  id in  (select parent from category where id in (select distinct parent from category where id in (select category_id from category_file where item_id=$id) ))
  OR
  id in (select parent from category where id in ( select parent from category where id in (select distinct parent from category where id in (select category_id from category_file where item_id=$id) )))
  OR
  id in (select parent from category where id in (select parent from category where id in ( select parent from category where id in (select distinct parent from category where id in (select category_id from category_file where item_id=$id) )))
  )  
)"
...
sqlite3 "$database" "insert into category_item (category_id,item_id,item_type_id) values (($dbquery),$id,3);" 2> /dev/null
...
rm_tag_file "$id" `cat $ramdisk/rm_tag_categorys`

if [[ $USE_HISTORY = y ]] ; then
sqlite3 $database "insert into history (item_id,item_type_id,categories,zones,timestamp,command) values ($id) ,3,`echo $category_name | sed "s/','/,/g"`,$zones,(select datetime()),'$SCRIPTNAME');"
fi
